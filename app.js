...
function renderDashboard() {
  ...
  const availableToday = ics.length - new Set(onLeaveToday.map(l => l.alias)).size;
  ...
  ics.forEach(a => {
    ...
    if (leave) {
      const cls = leave.type === 'halfday' ? 'half' : 'on-leave';
  ...
  const perA = {};
  ics.forEach(a => { perA[a] = [0, 0, 0]; });
  leaves.forEach(l => { if (!perA[l.alias]) return; const ti = LEAVE_TYPES.findIndex(t => t.id === l.type); if (ti >= 0) perA[l.alias][ti] += l.days; });
  const maxV = Math.max(...Object.values(perA).map(v => v[0] + v[1] + v[2]), 1);
  const sorted = Object.entries(perA).sort((a, b) => (b[1][0] + b[1][1] + b[1][2]) - (a[1][0] + a[1][1] + a[1][2]));
  ...
  sorted.forEach(([a, v]) => {
    const t = v[0] + v[1] + v[2]; if (t === 0) return;
  ...
  // Daily tracker summary for Akhila
  if (state.mgr === 'aggannam') {
...
// ========== DAILY SNAPSHOT (for Akhila's dashboard) ==========
function renderDailySnapshot() {
  const today = new Date().toISOString().slice(0, 10);
  const tracker = db.dailyTracker['aggannam'] || {};
  const ics = ORG['aggannam'].directs;
...
function renderSpreadsheet() {
  ...
  h += '<span style="color:var(--muted)"><strong>WO</strong>=Week Off</span><span style="color:var(--purple)"><strong>H</strong>=Holiday</span>';
  h += '</div>';
  ...
  ics.forEach(a => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      else if (val === 'H') bg = 'background:#f3e8ff';
...
// ========== DAILY TRACKER (Akhila's Team) ==========
function renderDailyTracker() {
  const mgr = 'aggannam';
  const ics = ORG[mgr].directs;
  const today = new Date().toISOString().slice(0, 10);
  ...
  ics.forEach(a => {
    ...
    if (!db.dailyTracker[mgr][a][today]) db.dailyTracker[mgr][a][today] = { status: 'present', notes: '' };
  });
  ...
  h += `<div class="stats">
    ...
    <div class="stat red"><div class="val">${absent}</div><div class="lbl">Absent</div></div>
    <div class="stat orange"><div class="val">${half}</div><div class="lbl">Half-day</div></div>
    <div class="stat"><div class="val">${ics.length}</div><div class="lbl">Total Team</div></div>
  </div>`;
  ...
  ics.forEach(a => {
    ...
    const badge = status === 'present' ? 'b-green' : status === 'absent' ? 'b-red' : 'b-orange';
    h += `<div class="daily-card ${cls}">
      <div class="dc-name">${info.name}</div>
      <div style="font-size:11px;color:var(--muted)">${a}</div>
      <div class="dc-status"><span class="badge ${badge}">${status.charAt(0).toUpperCase() + status.slice(1)}</span></div>
    </div>`;
  });
  h += '</div>';

  // Editable table
  h += '<table><thead><tr><th>Alias</th><th>Name</th><th>Status</th><th>Notes</th></tr></thead><tbody>';
  ics.forEach(a => {
    const info = ORG[a] || { name: a };
    const rec = db.dailyTracker[mgr][a] && db.dailyTracker[mgr][a][today];
    const status = rec ? rec.status : 'present';
    ...
    h += `<tr>
      ...
      <td>${info.name}</td>
      <td><select id="dt-status-${a}" onchange="updateDailyStatus('${a}', this.value)">
        <option value="present" ${status === 'present' ? 'selected' : ''}>Present</option>
        <option value="absent" ${status === 'absent' ? 'selected' : ''}>Absent</option>
        <option value="halfday" ${status === 'halfday' ? 'selected' : ''}>Half-day</option>
      </select></td>
      <td><input type="text" id="dt-notes-${a}" value="${notes}" placeholder="Add notes..." style="width:100%;padding:4px;border:1px solid var(--border);border-radius:3px;font-size:12px" onchange="updateDailyNotes('${a}', this.value)"></td>
    </tr>`;
  });
  h += '</tbody></table></div>';
...
function updateDailyStatus(alias, status) {
  ...
  if (!db.dailyTracker[mgr][alias][date]) db.dailyTracker[mgr][alias][date] = { status: 'present', notes: '' };
  db.dailyTracker[mgr][alias][date].status = status;
  save();
  // Refresh just the summary stats
  render();
}
...
function changeTrackerDate(date) {
  ...
  ics.forEach(a => {
    if (!db.dailyTracker[mgr][a]) db.dailyTracker[mgr][a] = {};
    if (!db.dailyTracker[mgr][a][date]) db.dailyTracker[mgr][a][date] = { status: 'present', notes: '' };
  });
  save();
  render();
}
...
function exportDailyCSV() {
  ...
  const ics = ORG[mgr].directs;
  let csv = 'Alias,Name,Status,Notes\n';
  ics.forEach(a => {
    const info = ORG[a] || { name: a };
    const rec = db.dailyTracker[mgr] && db.dailyTracker[mgr][a] && db.dailyTracker[mgr][a][date];
    csv += `${a},${info.name},${rec ? rec.status : 'present'},"${rec ? rec.notes : ''}"\n`;
  });
  downloadCSV(csv, `Akhila_Daily_${date}.csv`);
}
...
function renderReports() {
  ...
  leaves.forEach(l => {
    ...
    if (!monthlyData[mk]) monthlyData[mk] = { planned: 0, unplanned: 0, halfday: 0 };
  ...
  h += '<table><thead><tr><th>Month</th><th>Planned</th><th>Unplanned</th><th>Half-day</th><th>Total</th></tr></thead><tbody>';
  Object.keys(monthlyData).sort().forEach(mk => {
    const d = monthlyData[mk];
    h += `<tr><td><strong>${mk}</strong></td><td class="num">${d.planned}</td><td class="num" style="${d.unplanned > 5 ? 'color:var(--danger)' : ''}">${d.unplanned}</td><td class="num">${d.halfday}</td><td class="num">${d.planned + d.unplanned + d.halfday}</td></tr>`;
  });
  h += '</tbody></table></div>';

  // Shrinkage calculation
  const totalWorkDays = ics.length * 22; // approx per month
  const totalLeaves = leaves.reduce((s, l) => s + l.days, 0);
  const shrinkage = totalWorkDays > 0 ? ((totalLeaves / totalWorkDays) * 100).toFixed(1) : 0;
  ...
  h += `<div class="card"><h2>Shrinkage Summary</h2>
    <div class="stats">
      ...
      <div class="stat ${parseFloat(shrinkage) > 15 ? 'red' : 'green'}"><div class="val">${shrinkage}%</div><div class="lbl">Shrinkage Rate</div></div>
    </div>
  </div>`;
